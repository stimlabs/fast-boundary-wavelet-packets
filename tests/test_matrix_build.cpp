...
// Verify A is sparse with shape (length, length) for various input sizes.
...
// Verify S is sparse with shape (length, length) for various input sizes.
...
static void test_a_lowpass_highpass_split() {
    ...
    auto a = construct_a(w, n, opts).to_dense();

    auto dec_lo = torch::tensor(w.dec_lo, opts);
    auto dec_hi = torch::tensor(w.dec_hi, opts);
    auto expected_lo = construct_strided_conv_matrix(dec_lo, n, 2).to_dense();
    auto expected_hi = construct_strided_conv_matrix(dec_hi, n, 2).to_dense();
